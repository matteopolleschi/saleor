...
def shipping_method_applicable(cart, method):
    ...
    if method.type == ShippingMethodType.PRICE_BASED:
        return value_in_range(
            ...
            maximum=method.maximum_order_price, value=cart.get_subtotal())
    elif method.type == ShippingMethodType.WEIGHT_BASED:
        return value_in_range(
            minimum=method.minimum_order_weight,
            maximum=method.maximum_order_weight, value=cart.get_total_weight())
...
def check_shipping_method(cart):
    ...
    if not shipping_method_applicable(cart, cart.shipping_method):
        return clear_shipping_method(cart)
    return True
